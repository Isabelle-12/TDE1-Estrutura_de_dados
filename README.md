...
| `feature/fila` | Gabriela | Classe `FilaPropria` (enfileirar/desenfileirar) |
| `feature/leitura-imagem` | Pedro | Leitura/escrita de imagem com `File` e `BufferedImage`, incluindo salvamento das imagens intermediárias para a animação |
| `feature/algoritmo-floodfill` | Laura | Núcleo do algoritmo Flood Fill (verificação de limites, cor de fundo, pintura e empilhamento/enfileiramento dos vizinhos) |
...
Depois que todas as branches estiverem na `master`, a classe `Main` é criada em conjunto, juntando tudo: leitura da imagem, execução do Flood Fill com Pilha e com Fila, e geração dos resultados/animação.
